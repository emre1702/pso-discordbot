...
export class ToUserTransferRequestService {
    ...
    private async notifyTargetUserAboutIncomingTransferRequest(
        ...
        const teamName = await this.databaseService.teams
            ...
            .then((team) => team!.name);
        const requesterUser = await container.client.users.fetch(requestingUserId);
        const requesterNameWithMention = requesterUser
            ? getUserMentionAndName(requesterUser)
            : tFunction("transfer:send-to-user:unknown-user");
        const guildName = (await container.client.guilds.fetch(guildId))?.name || tFunction("transfer:send-to-user:unknown-guild");

        const message = tFunction("transfer:send-to-user:notification", {
            guildName,
            teamName,
    ...
    async respondToUserTransferRequest(
        ...
        if (response === transfer_request_status.accepted) {
            ...
            await this.transferChannelService.sendTransferMessage(userId, teamId, previousTeamIdAndRole?.team_id, guildId);
